...
void LCD_nokia_init(void)
{
	...
	PORT_SetPinMux(PORTB, DATA_OR_CMD_PIN, kPORT_MuxAsGpio);
	PORT_SetPinMux(PORTB, RESET_PIN, kPORT_MuxAsGpio);
	
	GPIO_PinInit(GPIOB, DATA_OR_CMD_PIN, &led_config);
...
void LCD_nokia_bitmap(const uint8_t bitmap[]){
	...
	GPIO_PortSet(GPIOB, 1u << DATA_OR_CMD_PIN);
...
void LCD_nokia_write_byte(uint8_t data_or_command, uint8_t data)
{
	...
	if(data_or_command)
		...
		GPIO_PortClear(GPIOB, 1u << DATA_OR_CMD_PIN);
